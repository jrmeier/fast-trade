...
def uppercase_ohlc(simple_ohlc):
    return simple_ohlc.rename({c: c.upper() for c in simple_ohlc.columns})


def _finite(series: pl.Series) -> pl.Series:
    """Drop null/NaN values so warmup checks can read a last finite sample."""
    out = series.drop_nulls()
    if out.dtype in (pl.Float32, pl.Float64):
        out = out.drop_nans()
        out = out.filter(out.is_finite())
    return out
...
def test_inputvalidator_missing_column_raises():
    df = pl.DataFrame({"open": [1], "high": [1], "low": [1]})
...
def test_inputvalidator_standalone_decorator():
    ...
    df = pl.DataFrame({"OPEN": [1], "HIGH": [1], "LOW": [1], "CLOSE": [1]})
    out = dummy(df)
    assert "close" in out.columns


def test_polars_input_returns_polars(simple_ohlc):
    """Polars OHLC in → Polars Series/DataFrame out."""
...
def test_sma_hand_check(simple_ohlc):
    ...
    assert sma[4] == pytest.approx(11.0)  # (12+11+10)/3
...
def test_bbands_ordering(simple_ohlc):
    ...
    row = bb.drop_nulls().row(-1, named=True)
...
def test_rolling_max_missing_column(ohlcv):
    with pytest.raises(LookupError, match="missing"):
        TA.ROLLING_MAX(ohlcv.clone(), column="missing")


def test_rolling_min_missing_column(ohlcv):
    with pytest.raises(LookupError, match="missing"):
        TA.ROLLING_MIN(ohlcv.clone(), column="missing")
...
def test_mfi_pos_neg_branches(simple_ohlc):
    ...
    assert _finite(out).is_between(0, 100).all()


def test_obv_all_change_types(ohlcv):
    out = TA.OBV(ohlcv.clone())
    assert out[-1] != 0
...
def test_sqzmi_squeeze_states(simple_ohlc):
    ...
    assert on.dtype == pl.Boolean or set(_finite(on).to_list()) <= {True, False}
...
def test_williams_fractal_detection(ohlcv):
    ...
    assert _finite(out.get_column("BearishFractal")).is_in([0.0, 1.0, True, False]).any()
    assert _finite(out.get_column("BullishFractal")).is_in([0.0, 1.0, True, False]).any()
